...
from mqtt_monitor.plugins.utils import (
    make_collector,
    run_command,
    run_command_int,
    run_command_tolerant,
)
...
def _pending_updates():
    count = run_command_int("apt list --upgradable 2>/dev/null | grep -c upgradable")
    return {"value": count, "unit": "packages"}


@collector("security_updates")
def _security_updates():
    count = run_command_int("apt list --upgradable 2>/dev/null | grep -i security | wc -l")
    return {"value": count, "unit": "packages"}


@collector("failed_services")
def _failed_services():
    count = run_command_int("systemctl --failed --no-legend --no-pager | wc -l")
    return {"value": count, "unit": "services"}


@collector("running_services")
def _running_services():
    count = run_command_int(
...
def _logged_in_users():
    count = run_command_int("who | wc -l")
    return {"value": count, "unit": "users"}


@collector("docker_running")
def _docker_running():
    count = run_command_int("docker ps -q 2>/dev/null | wc -l")
    return {"value": count, "unit": "containers"}


@collector("docker_total")
def _docker_total():
    count = run_command_int("docker ps -aq 2>/dev/null | wc -l")
    return {"value": count, "unit": "containers"}


@collector("ufw_status")
def _ufw_status():
    output = run_command_tolerant("ufw status 2>/dev/null | head -1")
...
@collector("last_boot")
def _last_boot():
    val = run_command("who -b | awk '{print $3, $4}'")
    return {"value": val, "unit": ""}


@collector("open_ports")
def _open_ports():
    count = run_command_int("ss -tuln | grep LISTEN | wc -l")
